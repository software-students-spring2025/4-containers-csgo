...
def test_analyze_happy_path():
    ...
    with app.test_client() as c, \
         patch("app.api.analyzer") as mock_an, \
         patch("app.api.DB_CONNECTED", False):     # skip Mongo
        mock_an.polarity_scores.return_value = {"compound": 0.9, "pos":1,"neg":0,"neu":0}
...
def test_api_analyze_success():
    ...
    from app import api  # local import so patches apply first

    with patch.object(api, "DB_CONNECTED", False), \
         patch.object(api.analyzer, "polarity_scores", return_value={
             "pos": 0.7, "neg": 0.0, "neu": 0.3, "compound": 0.7
         }):
...
def test_analyze_db_connected_stores_data():
    ...
    with patch.object(api, "DB_CONNECTED", True), \
         patch.object(api.analyses, "insert_one") as mock_insert, \
         patch.object(api.analyzer, "polarity_scores", return_value={
...
def test_analyze_db_error():
    ...
    with patch.object(api, "DB_CONNECTED", True), \
         patch.object(api.analyses, "insert_one", side_effect=Exception("Mock DB fail")), \
         patch.object(api.analyzer, "polarity_scores", return_value={
             "pos":0.3, "neg":0.0, "neu":0.7, "compound":0.3
         }):
        client = api.app.test_client()
